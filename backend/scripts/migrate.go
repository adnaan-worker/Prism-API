...
func main() {
	...
	err = db.Exec(`
		CREATE TABLE IF NOT EXISTS account_credentials (
			...
			usage_last_updated TIMESTAMP,
			base_limit INTEGER DEFAULT 0,
			base_current INTEGER DEFAULT 0,
			free_trial_limit INTEGER DEFAULT 0,
			free_trial_current INTEGER DEFAULT 0,
...
func createIndexes(db *gorm.DB) {
	indexes := []string{
		...
		"CREATE INDEX IF NOT EXISTS idx_account_credentials_status ON account_credentials(status)",
		...
		"CREATE INDEX IF NOT EXISTS idx_account_credentials_subscription_type ON account_credentials(subscription_type)",
		"CREATE INDEX IF NOT EXISTS idx_account_credentials_usage_percent ON account_credentials(usage_percent)",
		// 账号池选择优化
